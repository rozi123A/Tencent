...
export const useAppStore = create<AppState>((set) => ({
  ...
  remoteUsers: [],
  inviteRemoteUsers: [],
  setSdkAppId: (val) => { try { localStorage.setItem('trtc_sdkAppId', val); } catch (e) {} set({ sdkAppId: val }); },
  setSdkSecretKey: (val) => { try { localStorage.setItem('trtc_sdkSecretKey', val); } catch (e) {} set({ sdkSecretKey: val }); },
  ...
  addSuccessLog: (content) => set((state) => ({ logs: [...state.logs, { type: 'success', content }] })),
  addFailedLog: (content) => set((state) => ({ logs: [...state.logs, { type: 'failed', content }] })),
  addRemoteUser: (user) => set((state) => ({ remoteUsers: [...state.remoteUsers, user] })),
  removeRemoteUser: (elementId) => set((state) => ({ remoteUsers: state.remoteUsers.filter((u) => u.elementId !== elementId) })),
  clearRemoteUsers: () => set({ remoteUsers: [] }),
  addInviteRemoteUser: (user) => set((state) => ({ inviteRemoteUsers: [...state.inviteRemoteUsers, user] })),
  removeInviteRemoteUser: (elementId) => set((state) => ({ inviteRemoteUsers: state.inviteRemoteUsers.filter((u) => u.elementId !== elementId) })),
